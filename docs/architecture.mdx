...
- `app/root.tsx`: layout raíz que envolve `<I18nProvider>`, `<ThemeProvider>` e o novo `<AuthProvider>`; define o skip link e monta o Document/Body com meta tags e `<Outlet />` do `react-router`.
...
- O loader raiz lê os cookies de acesso, injeta o estado inicial no `AuthProvider` e mantém o header/rotas sincronizados com o backend Passport/JWT.
